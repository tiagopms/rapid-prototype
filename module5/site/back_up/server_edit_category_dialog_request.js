...
var is_logged 	  = functions.is_logged,
	xss		      = functions.xss,
	valid_entries = functions.valid_entries;

var edit_category_dialog_request = function(socket, mysql, check, sanitize, querystring) {
	socket.on("edit_category_dialog_request", function( data ) {
		is_logged(socket.user.is_logged, socket);	
		if (!socket.uset.is_logged) {
			return;
		}
		...
		mysql.query(
			...
			function(err, result, fields) {
				...
				mysql.query(
					...
					function(err, result, fields) {
						if (err) {
							socket.emit('error', {"message": "Invalid sql query 2: "+ err, "code": "21"});
							return;
						} 
						var words = [];
						for (var i in result) {
