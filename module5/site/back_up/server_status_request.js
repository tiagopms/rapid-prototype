...
var status_request = function(socket, mysql, check, sanitize, querystring) {
	socket.on( 'status_request', function( data ) {
		...
		if(!socket.user.is_logged) {
			...
			status_response.user = {"username": socket.user.username, "gravatar": crypto.createHash('md5').update(socket.user.email).digest("hex"), "admin": socket.user.admin, "token": socket.user.token };
