...
function EventXY(e, id) {
  if(Defined(e.offsetX)) {
    ...
    if(e.pageX)
      return {x: e.pageX - p.left, y: e.pageY - p.top};
  }
  return null;
}
function MouseOut(e) {
...
TCproto.TooltipDiv = function(active,tag) {
  var tc = this, s = tc.ttdiv.style, none = 'none';
  if(active && tag.title) {
...
TagCanvas.Start = function(jlist, options, lctr) {
  var links, ctr = lctr ? jQuery('#'+lctr) : jlist;
  if(doc.all && !lctr) return false; // IE must have external list
  links = ctr.find('a');
  if(Defined(window.G_vmlCanvasManager)) {
    jlist.each(function() { $(jlist)[0] = window.G_vmlCanvasManager.initElement($(jlist)[0]); });
    options.ie = parseFloat(navigator.appVersion.split('MSIE')[1]);
  }

  if(!links.length || !jlist[0].getContext || !jlist[0].getContext('2d').fillText)
    return false;

  jlist.each(function() {
    var i, vl, im, ii, tag, jqt, w, weights = [], shape, shapeArgs,
      pfuncs = {
    ...
    jqt.z2 = jqt.z1 / jqt.zoom;

    jqt.radius = min(this.height, this.width) * 0.37 *
      (jqt.z2 + jqt.z1) / jqt.z1;
    jqt.max_radius = jqt.radius * max(jqt.radiusX,
      max(jqt.radiusY, jqt.radiusZ));
    ...
    jqt.textHeight *= 1;
    jqt.pulsateTo = Clamp(jqt.pulsateTo, 0, 1);
    jqt.minBrightness = Clamp(jqt.minBrightness, 0, 1);
    jqt.maxBrightness = Clamp(jqt.maxBrightness, jqt.minBrightness, 1);
    ...
    jqt.dx = jqt.dy = 0;
    jqt.touched = 0;
    jqt.Animate = jqt.dragControl ? jqt.AnimateDrag : jqt.AnimatePosition;
    ...
    shapeArgs = jqt.shape.toString().split(/[(),]/);
    shape = shapeArgs.shift();
    jqt.shape = pfuncs[shape] || pfuncs.sphere;
    shapeArgs = [links.length, jqt.radiusX, jqt.radiusY, jqt.radiusZ].concat(shapeArgs);
    vl = jqt.shape.apply(jqt, shapeArgs);
    ...
    if(!jqt.noMouse && !handlers[$(this)[0].id]) {
      ...
      AddHandler('mouseup', MouseUp, this);
      if(jqt.dragControl) {
        AddHandler('mousedown', MouseDown, this);
        AddHandler('touchstart', TouchDown, this);
        AddHandler('touchend', TouchUp, this);
        AddHandler('touchcancel', TouchUp, this);
        AddHandler('touchmove', TouchMove, this);
        AddHandler('selectstart', Nop, this);
      }
      if(jqt.wheelZoom) {
...
jQuery.fn.tagcanvas = function(options, lctr) {
  var fn = {
   pause: function() {
     $(this).each(function() {
       TagCanvas.tc[$(this)[0].id] && TagCanvas.tc[$(this)[0].id].Pause();
     });
   },
   resume: function() {
     $(this).each(function() {
       TagCanvas.tc[$(this)[0].id] && TagCanvas.tc[$(this)[0].id].Resume();
     });
   }
  };
  if(typeof(options) == 'string' && fn[options]) {
    ...
    return TagCanvas.Start(this, options, lctr);
  }
};
